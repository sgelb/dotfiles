...
def arguments():
    parser = argparse.ArgumentParser(description="Symlink dotfiles.")
    parser.add_argument(
        "-f",
        "--force",
        dest="force",
...
if __name__ == "__main__":
    ...
    for source, target in sorted(dotfiles.items(), key=lambda x: x[0].lower()):
        src = os.path.join(dotfile_folder, source)
